...
namespace RxyEx.Bots.DotNet
{
    public abstract class BaseBot
    {
        ...
        protected BigInteger RyxExETH { get; set; }
        protected BigInteger WalletETH { get; set; }
        protected BigInteger RyxExToken { get; set; }
        ...
        private async Task<BigInteger> GetRyxExBalance(string token, string user)
        {
            BigInteger balance = 0;
            try
            {
                balance = (BigInteger) await this.Service.GetRyxExBalance(token, user);
            }
        ...
        private void PrintWallet()
        {
            ...
            Console.WriteLine($"RyxEx ETH balance:     {uc.FromWei(this.RyxExETH):N18}");
            Console.WriteLine($"Wallet token balance:       {uc.FromWei(this.WalletToken):N18}");
            Console.WriteLine($"RyxEx token balance:   {uc.FromWei(this.RyxExToken):N18}");
        }
