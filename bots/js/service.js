...
const ABIRyxEx = require('../contracts/ryxex.json');
...
function Service() {
  ...
  self.init = config => new Promise((resolve, reject) => {
    ...
    self.contractRyxEx = self.web3.eth.contract(ABIRyxEx).at(config.addressRyxEx);
  ...
  self.getRyxExBalance = (token, user) => new Promise((resolve, reject) => {
    self.contractRyxEx.balanceOf(token === 'ETH' ? '0x0000000000000000000000000000000000000000' : token.addr, user.addr, (err, result) => {
      if (err) reject(err);
      resolve(result);
    });
  });
  ...
  self.takeOrder = (user, order, fraction) => new Promise((resolve, reject) => {
    ...
    .then((nonce) => {
      ...
      self.contractRyxEx.testTrade.call(
        order.tokenGet,
        order.amountGet,
        order.tokenGive,
        order.amountGive,
        ...
        (errTest, resultTest) => {
          ...
          const data = self.contractRyxEx.trade.getData(
            order.tokenGet,
            order.amountGet,
            order.tokenGive,
            order.amountGive,
  ...
  self.createOrder = (side, expires, price, amount, token, user) => {
    ...
    const contractAddr = self.config.addressRyxEx;
