#!/usr/bin/python3
#
# ryxexclientservice.py: RyxEx API Client Service for python3 using websockets
# ===============================================================================
#
...
# RyxEx contract address
# This rarely changes.
addressRyxEx = '0x56a20f5c2ad2d88d635783780a7a1ad0e50df492'    # ryxex_2's contract address
# Global API interfaces
web3 = Web3(HTTPProvider('https://mainnet.infura.io/GxiQX2aLNKrlJJH0mRZe'))

class RyxExClientService:
    ...
    def printBalances(self, token, userAccount):
        ...
        print("RyxEx ETH balance: %.18f ETH" % self.getRyxExBalance('ETH', userAccount))
        print("RyxEx token balance: %.18f tokens" % self.getRyxExBalance(token, userAccount))
    ...
    def on_message(self, ws, message):
        ...
        if 'market' in j:
            ...
            if 'trades' in market:
                ...
                print("WARNING: no trades found in market response from RyxEx API, this happens from time to time but we don't really need it here so not retrying.")
            # Fill the list of orders
            if 'orders' in market:
                print("INFO: market reply contains orderbook")
                self.updateOrders(j[1]['orders'])
            else:
                print("WARNING: market response from RyxEx API did not contain order book, this happens from time to time, retrying after a 10 second grace period...")
                time.sleep(10)
                self.send_getMarket()
        elif 'orders' in j:
            ...
            print("Received funds event from RyxEx API, no action to take.")
        elif 'messageResult' in j:
            messageResult = j[1]
            print("Received messageresult from RyxEx API, result is: " + str(messageResult))
        else:
            print("Received an unrecognized event from the RyxEx API, no action to take.")
    ...
    def on_pong(self, ws, pong):
        print('RyxEx WebSocket API replied to our ping with a pong:' + str(pong))
    ...
    def on_open(self, ws):
        #print("RyxEx WebSocket connected")
        if len(self.orders_sells) == 0:
            self.send_getMarket()

    def websocket_connect(self):
        self.ws = websocket.WebSocketApp(
            "wss://socket.ryxex.com/socket.io/?transport=websocket",
    ...
    def start(self, userAccount, token):
        ...
        def run(*args):
            ...
            while True:
                time.sleep(10)
                print("RyxExClientService thread running...")
        _thread.start_new_thread(run, ())

    def __init__(self):
        global addressRyxEx

        addressRyxEx = Web3.toChecksumAddress(addressRyxEx)

        # Load the ABI of the RyxEx contract
        with open('../contracts/ryxex.json', 'r') as abi_definition:
            abiRyxEx = json.load(abi_definition)
        self.contractRyxEx = web3.eth.contract(address=addressRyxEx, abi=abiRyxEx)
